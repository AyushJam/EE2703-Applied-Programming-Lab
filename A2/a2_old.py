...
class ccvs:
    def __init__(self, name, toNode, fromNode, currentThroughWhichVolSource, value):
        ...
        self.value = value  # Transresistance
...
try:
    with open(argv[1]) as f:
        ...
        try:
            ...
            result = {}
            for i in range(n):
                result["Voltage at node {}".format(i)] = str(round(float(x[i].real), DIGITS_AFTER_DECIMAL)) + '+' + str(
                    round(float(x[i].imag), DIGITS_AFTER_DECIMAL)) + 'j'
            for i in range(len(voltage_sources)):
                result["Current through VS {}".format(voltage_sources[i].name)] = str(
                    round(float(x[n + i].real), DIGITS_AFTER_DECIMAL)) + '+' + str(round(float(x[n + i].imag), DIGITS_AFTER_DECIMAL)) + 'j'
            for i in range(len(vol_ctrl_vol_sources)):
                result["Current through VCVS {}".format(vol_ctrl_vol_sources[i].name)] = round(
                    (x[n + i + len(voltage_sources)]), DIGITS_AFTER_DECIMAL)
