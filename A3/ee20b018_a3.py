...
# in each iteration, I will get the best estimate (minimized norm) of A and B
# as [A, B]. Every time this gets added to the list by comprehension
# np.array converts it to a matrix
error_in_A = A0 - best_estimate[:, 0]  # first column of best_estimate will be for As
error_in_B = B0 - best_estimate[:, 1]  # second one for B

# Error vs Noise plot
plot(sigma, abs(error_in_A), 'ro', label='Aerr', linestyle='dashed', linewidth='1')
# ro is used to plot with red circle markers
plot(sigma, abs(error_in_B), 'go', label='Berr', linestyle='dashed', linewidth='1')
...
loglog(sigma, abs(error_in_A), 'ro', label='Aerr')
stem(sigma, abs(error_in_A), '-ro')
stem(sigma, abs(error_in_B), '-go')
loglog(sigma, abs(error_in_B), 'go', label='Berr')
xlabel(r'Noise standard deviation $\rightarrow$')
ylabel(r'MS error$\rightarrow$')
